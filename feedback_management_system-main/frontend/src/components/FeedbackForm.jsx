...
const FeedbackForm = () => {
  ...
  const loadSubjects = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `/subjects/${session}/${semester}/${batch}`
      );
  ...
  const handleOverallDataChange = (subjectCode, field, value) => {
    setOverallData((prev) => {
      ...
      updated[subjectCode].overall = calculateAverageRating(subjectCode).toString();
      return updated;
    });
  };
  ...
  const submitFeedback = async () => {
    ...
    try {
      const statusRes = await axios.get(`/feedback-status/${rollNo}`);
      if (statusRes.data.submitted) {
        setIsError(true);
        setErrorMsg("You have already submitted feedback");
        return;
      }
    } catch (err) {
      setIsError(true);
      setErrorMsg("Error checking submission status");
      return;
    }
