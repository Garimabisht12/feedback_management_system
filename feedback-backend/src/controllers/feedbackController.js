...
const checkFeedbackStatus = async (req, res) => {
  try {
    ...
    const hasSubmitted = await Student.findOne({ rollNumber: Number(rollNumber) }).select('hasSubmittedFeedback');
  ...
  try {
    ...
    Object.keys(ratings).forEach(key => {
      if (typeof ratings[key] === 'string') {
        ratings[key] = JSON.parse(ratings[key]);
      }

      ratings[key] = ratings[key].map(Number);
    });

    Object.keys(overallData).forEach(key => {
      overallData[key] = {
        syllabus: Number(overallData[key].syllabus),
        voice: Number(overallData[key].voice),
        regularity: Number(overallData[key].regularity)
      };
    });
    ...
    await feedback.save();
    console.log('Feedback saved, updating student...');
    
    await Student.findOneAndUpdate({ rollNo: studentRoll }, { hasSubmittedFeedback: true });
    console.log('Student updated, sending response...');
    ...
    feedbacks.forEach(feedback => {
      if (feedback.overallData) {
        Object.values(feedback.overallData).forEach(data => {
          if (data && data.syllabus) overallMetrics.syllabus.push(parseInt(data.syllabus) || 0);
          if (data && data.voice) overallMetrics.voice.push(parseInt(data.voice) || 0);
          if (data && data.regularity) overallMetrics.regularity.push(parseInt(data.regularity) || 0);
        });
      }
    });
