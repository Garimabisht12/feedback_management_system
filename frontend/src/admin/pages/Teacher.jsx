        ...
        <div className="space-y-3">
          {data.map((item, index) => (
            <div key={index} className="flex items-center gap-3">
              <div className="w-48 text-sm text-gray-700" title={item.parameter || item.subjectCode}>
                {item.parameter || item.subjectCode}
              </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              ...
              <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
                ...
                <p className="text-4xl font-bold">{analytics.overallRatingAvg?.toFixed(2) || 0}/5</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-6 text-white">
                ...
                <p className="text-4xl font-bold">{analytics.averageMetrics?.syllabus?.toFixed(1) || 0}/10</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <p className="text-3xl font-bold text-purple-600">{analytics.averageMetrics?.voice?.toFixed(1) || 0}/10</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-2">Regularity</h3>
                <p className="text-3xl font-bold text-green-600">{analytics.averageMetrics?.regularity?.toFixed(1) || 0}/10</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-2">Syllabus Coverage</h3>
                <p className="text-3xl font-bold text-orange-600">{analytics.averageMetrics?.syllabus?.toFixed(1) || 0}/10</p>
              </div>
            </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
                          <th className="px-6 py-3 text-left text-sm font-semibold">Subject Code</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {analytics.subjectBreakdown.map((subject, index) => (
                          <tr key={index} className="hover:bg-blue-50 transition-colors duration-200">
                            <td className="px-6 py-4 text-gray-900 font-medium">{subject.subjectCode}</td>
                            <td className="px-6 py-4">
                              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-800">
                                {subject.averageRating.toFixed(2)} / 5
                              </span>
                            </td>
            ...
            {/* Comments Section */}
            {analytics.teacherComments && analytics.teacherComments.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Student Comments</h3>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {analytics.teacherComments.map((item, index) => (
                    <div key={index} className="border-l-4 border-blue-500 bg-blue-50 p-4 rounded-lg hover:bg-blue-100 transition-colors duration-200">
                      ...
                      <p className="text-xs text-gray-500">{item.session} - Semester {item.semester}</p>
                    </div>
                  ))}
                </div>
              </div>
